...
// Privacy helper - checks if user is admin
const isAdmin = (req) => {
  const user = req.user;
...
const userManagementController = {
  ...
  getAllUsers: async (req, res) => {
    try {
      ...
      if (!isAdmin(req)) {
        console.log('🔒 Non-admin user - filtering to own record only');
        
        // For non-admins, only return their own user record
        const user = await User.findById(req.user.id);
        if (!user) {
          return res.status(404).json(createResponse(false, 'User not found'));
        }

        // Apply search filter if provided (only relevant for their own username)
  ...
  getUserById: async (req, res) => {
    try {
      ...
      // Privacy check: Regular users can only access their own record
      if (!isAdmin(req) && userId !== req.user.id) {
        return res.status(403).json(
          createResponse(false, 'Access denied. You can only view your own user information.')
        );
      }
  ...
  createUser: async (req, res) => {
    try {
      ...
      if (!isAdmin(req)) {
        return res.status(403).json(
          createResponse(false, 'Access denied. Only administrators can create users.')
        );
      }
