-- =====================================================
-- ROLES AND PERMISSIONS SYSTEM DATABASE SCHEMA (FIXED)
...
-- =====================================================
-- 1. UPDATE EXISTING ROLES TABLE
-- Add missing columns to existing roles table
-- =====================================================
ALTER TABLE roles ADD COLUMN IF NOT EXISTS level INT DEFAULT 1 COMMENT 'Role hierarchy level (1=lowest, 10=highest)';
ALTER TABLE roles ADD COLUMN IF NOT EXISTS is_system_role TINYINT(1) DEFAULT 0 COMMENT '1 if system role (cannot be deleted)';

-- Update existing roles with hierarchy levels
UPDATE roles SET level = 10, is_system_role = 1 WHERE name = 'super_admin';
UPDATE roles SET level = 8, is_system_role = 1 WHERE name = 'admin';
UPDATE roles SET level = 6, is_system_role = 1 WHERE name = 'manager';
UPDATE roles SET level = 1, is_system_role = 1 WHERE name = 'user';
UPDATE roles SET level = 1, is_system_role = 0 WHERE name = 'viewer';
...
-- Super Admin gets ALL permissions (using new role names)
INSERT IGNORE INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r, permissions p
WHERE r.name IN ('Super Admin', 'super_admin');
...
-- View to see all user permissions (fixed for existing username column)
...
-- View to see role permissions summary (fixed for level column)
...
ORDER BY COALESCE(r.level, 1) DESC, r.name, m.module_name;

-- =====================================================
-- CREATE INDEXES FOR BETTER PERFORMANCE
-- =====================================================
CREATE INDEX IF NOT EXISTS idx_user_permissions_composite ON user_roles (user_id, role_id, is_active);
CREATE INDEX IF NOT EXISTS idx_role_permissions_composite ON role_permissions (role_id, permission_id);
CREATE INDEX IF NOT EXISTS idx_permissions_composite ON permissions (module_id, permission_key, is_active);
