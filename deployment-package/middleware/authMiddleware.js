...
const requirePermission = (requiredPermission) => {
  return (req, res, next) => {
    ...
    // Super admin bypass (by name variants or level >= 10)
    const isSuperAdmin = req.user && req.user.role && (
      (typeof req.user.role.level === 'number' && req.user.role.level >= 10) ||
      ['super_admin','SuperAdmin','Super Admin','superadmin','SUPERADMIN'].includes(req.user.role.name)
    );
    if (isSuperAdmin) {
      console.log('🔥 SuperAdmin access granted (RBAC bypass)', requiredPermission);
      return next();
    }
...
const requireAnyPermission = (requiredPermissions) => {
  return (req, res, next) => {
    ...
    // Super admin bypass (by name variants or level >= 10)
    const isSuperAdminAny = req.user && req.user.role && (
      (typeof req.user.role.level === 'number' && req.user.role.level >= 10) ||
      ['super_admin','SuperAdmin','Super Admin','superadmin','SUPERADMIN'].includes(req.user.role.name)
    );
    if (isSuperAdminAny) {
      console.log('🔥 SuperAdmin access granted (RBAC bypass)', requiredPermissions);
      return next();
    }
